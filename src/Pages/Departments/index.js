...
class Departments extends Component {
    ...
    async handleClick(event, id, sub){
        ...
        if(getCookie("slug")!== null){
            let payload = {}
            var res = await store.getRestaurantFoodItems(payload, getCookie("slug"));
        }
        else{
                        ...
                        <div className="col-md-3 col-lg-3 col-xs-12">
                            ...
                            {this.state.mainCategories !== null && this.state.mainCategories.map((item, index) => (
                                        ...
                                        <div className="col-12 home__postsSection__sectionSubTitle text-left ">
                                            <Link to="#" onClick={(e)=> {this.handleClick(e, item.id, false)}} className="blacked">
                                                {item.name}
                                            </Link>
                                            { this.state.selectedCategory == item.id &&
                                            <img onClick={(e)=> {this.handleClick(e, item.id, false)}} src={downArrow} alt={downArrow} className="home__postsSection__sectionSubTitle__icon text-right " />
                                            }
                                            { this.state.selectedCategory != item.id &&
                                             <img onClick={(e)=> {this.handleClick(e, item.id, false)}} src={downArrow} alt={downArrow} className="home__postsSection__sectionSubTitle__icon text-right up" />
                                            }
                                            { this.state.selectedCategory == item.id &&
                                            <ul id={item.id} className="show" >
                                            {this.state.subCategories !== null && this.state.subCategories.map((item2, index2) => (
                                                <li>
                                                    <a href="javascript:;" onClick={(e)=> {this.handleClick(e, item2.id, true)}} >
                                                        {item2.name}
                                                    </a>
                                                </li>
                                            ))    
                                            ...
                                            <div className="col-md-6 col-lg-4 col-xs-12">
                                                <div className="home__postsSection__categoryBoxes">
                                                    ...
                                                    <div className="home__postsSection__categoryBoxes__itemImageBox">
                                                        <img src={`http://3.80.123.181${item.item_image[0].path}`} alt={elma}
                                                    ...
                                                    <div className="home__postsSection__categoryBoxes__itemPriceBox">
                                                        <span className="home__postsSection__categoryBoxes__itemPriceBox__price"> $ {item.price}
                                        </span>
